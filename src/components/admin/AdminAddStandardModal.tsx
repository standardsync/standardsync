...
interface AdminAddStandardModalProps {
  existingKeys: string[];
  onConfirm: (data: {
    key: string;
...
export function AdminAddStandardModal({ existingKeys, onConfirm, onCancel }: AdminAddStandardModalProps) {
  const [standardNumber, setStandardNumber] = useState('');
  const [name, setName] = useState('');
  const [description, setDescription] = useState('');
  const [version, setVersion] = useState('');

  // key는 standardNumber에서 자동 생성: "ISO 9001:2015" → "iso9001"
  const derivedKey = standardNumber
    .toLowerCase()
    .replace(/[^a-z0-9]/g, '')
    .replace(/^iso/, 'iso');

  const isDuplicate = existingKeys.includes(derivedKey);
  const canSubmit =
    standardNumber.trim() !== '' &&
    name.trim() !== '' &&
    derivedKey !== '' &&
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onCancel}>
      ...
      >
        <h3 className="text-lg font-bold text-gray-900 mb-4">새 표준 추가</h3>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="예: ISO 9001:2015, KS Q 8001:2023"
              autoFocus
            />
            {derivedKey && (
              <p className={`text-xs mt-1 ${isDuplicate ? 'text-red-500' : 'text-gray-400'}`}>
                키: {derivedKey}
                {isDuplicate && ' (이미 존재하는 표준입니다)'}
              </p>
            )}
          </div>
          <div>
